...
.stButton > button {
    background: #1e2130 !important; color: #c8cdd8 !important;
    border: 1px solid #2e3347 !important; border-radius: 8px !important;
    font-weight: 500 !important; transition: background 0.15s, border-color 0.15s !important;
}
.stButton > button:hover { background: #252a3d !important; border-color: #00d4aa !important; color: #e2e8f0 !important; }

.stFormSubmitButton > button {
    background: linear-gradient(135deg, #00b894, #00d4aa) !important;
    color: #0f1117 !important; border: none !important;
    font-weight: 700 !important; letter-spacing: 0.05em !important; border-radius: 8px !important;
}
.stFormSubmitButton > button:hover { background: linear-gradient(135deg, #00c9a7, #00e5bb) !important; color: #0f1117 !important; }
...
.stTextArea > div > div > textarea:focus {
    border-color: #00d4aa !important; box-shadow: 0 0 0 2px rgba(0,212,170,0.12) !important;
}
...
[data-baseweb="popover"] [role="listbox"], [data-baseweb="menu"] ul {
    background: #1a1d2e !important; border: 1px solid #2a2d3a !important; border-radius: 8px !important;
}
...
.stNumberInput button {
    background: #1e2130 !important; color: #9aa0b0 !important; border: 1px solid #2a2d3a !important;
}

label, .stTextInput label, .stNumberInput label,
.stSelectbox label, .stTextArea label, .stDateInput label {
    color: #9aa0b0 !important; font-size: 0.8rem !important; font-weight: 500 !important;
}

[data-testid="stExpander"] details summary {
    background: #13151f !important; border: 1px solid #2a2d3a !important;
    border-radius: 10px !important; color: #e2e8f0 !important;
    font-weight: 600 !important; padding: 12px 16px !important; list-style: none !important;
}
[data-testid="stExpander"] details[open] summary { border-radius: 10px 10px 0 0 !important; border-bottom-color: transparent !important; }
[data-testid="stExpander"] details summary:hover { background: #1a1d2e !important; border-color: #00d4aa !important; }
[data-testid="stExpander"] details summary p,
[data-testid="stExpander"] details summary span { color: #e2e8f0 !important; }
[data-testid="stExpander"] details > div[data-testid="stExpanderDetails"] {
    background: #0d0f18 !important; border: 1px solid #2a2d3a !important;
...
[data-testid="metric-container"] {
    background: #13151f !important; border: 1px solid #1e2130 !important;
...
[data-testid="stAlert"] {
    background: #1a1d2e !important; border: 1px solid #2a2d3a !important;
    border-radius: 8px !important; color: #9aa0b0 !important;
}
...
def crossover_triggered(direction: str, entry_price: float, live_price: float) -> bool:
    """
    Returns True when a pending trade's live price crosses its entry price.
    LONG:  triggered when live >= entry  (price rose to meet limit)
    SHORT: triggered when live <= entry  (price fell to meet limit)
    """
...
def load_data():
    ...
    for sheet_idx, r in enumerate(open_rows):
        ...
        if direction == "LONG":
            ...
            unrealized_pnl = (entry_price - live_price) * shares
    ...
    for sheet_idx, r, live_price in auto_activated:
        try:
            ...
            sh.update_row("Open Positions", sheet_idx + 2, updated)
            sh.append_row("Trade Log", [
                timestamp_str(), r[0], "AUTO-ACTIVATE", r[1].strip().upper(),
...
with left:
    ...
    with st.expander("⚡ Enter New Trade", expanded=True):
        ...
        nt_ticker_raw = st.text_input(
            "Ticker *", placeholder="AAPL", key="nt_ticker", label_visibility="collapsed",
        ).strip().upper()
        ...
        if nt_ticker_raw:
            ...
            if _live_quote:
                ...
                _mcap = f"${_live_quote['market_cap']/1e9:.1f}B" if _live_quote.get("market_cap") else "--"
                st.markdown(f"""
                <div style="background:#13151f;border:1px solid #2a2d3a;border-radius:10px;
                            padding:12px 18px;margin:6px 0 10px 0">
                    <span style="font-family:'JetBrains Mono',monospace;font-size:1rem;font-weight:700;color:#e2e8f0">{_live_quote['ticker']}</span>
                    <span style="color:#6b7185;font-size:0.82rem;margin-left:10px">{_live_quote['name']}</span>&nbsp;&nbsp;
                    <span style="font-family:'JetBrains Mono',monospace;font-size:1.25rem;font-weight:700;color:#e2e8f0">${_live_quote['price']:.2f}</span>
                    <span style="font-family:'JetBrains Mono',monospace;font-size:0.88rem;font-weight:600;color:{_chg_color};margin-left:10px">
                        {_chg_sign}{_live_quote['change']:.2f} ({_chg_sign}{_live_quote['change_pct']:.2f}%)
                    </span><br>
                    <span style="font-size:0.73rem;color:#6b7185">
                        Prev Close ${_live_quote['prev_close']:.2f} &nbsp;·&nbsp; Mkt Cap {_mcap} &nbsp;·&nbsp; {_live_quote.get('sector','')}
                    </span>
                </div>
                """, unsafe_allow_html=True)
            else:
        ...
        with st.form("new_trade_form", clear_on_submit=True):
            ...
            entry_price = c2.number_input("Entry Price *", min_value=0.01, step=0.01, format="%.2f", value=_default_ep)
        ...
        if submitted:
            ...
            if entry_price <= 0:  errors.append("Entry price must be > 0.")
            ...
            if errors:
                ...
                try:
                    ...
                    sh.append_row("Trade Log", [
                        timestamp_str(), trade_id, "PENDING", ticker,
                        f"{direction} {int(shares)} shares @ ${entry_price:.2f} | Cost: {fmt_usd(cost_basis)} | Awaiting price crossover"
                    ])
                    st.success(f"⏳ {direction} {ticker} entered as PENDING — will activate when live price {'rises to' if direction == 'LONG' else 'falls to'} ${entry_price:.2f}")
                    st.cache_data.clear()
                    st.cache_resource.clear()
                    st.rerun()
    ...
    if not d["positions"]:
        st.info("📭 No active positions yet. Trades start as pending until the price crossover is hit.")
    else:
            ...
            with st.container():
                    ...
                    if confirm_close:
                        if exit_price <= 0:
                            ...
                            try:
                                ...
                                sh.delete_row("Open Positions", p["sheet_idx"])
                                sh.append_row("Trade Log", [
                ...
                if st.session_state.get(f"show_edit_{p['trade_id']}"):
                    ...
                    if save_edit:
                        ...
                        try:
                            sh.update_row("Open Positions", p["sheet_idx"] + 2, updated)
    ...
    if d["pending_positions"]:
        ...
        st.markdown(
            ...
            f'<span style="color:#6b7185;font-size:0.65rem;margin-left:8px">· awaiting price crossover · excluded from portfolio value</span>'
            f'</div>',
            unsafe_allow_html=True,
        )
        ...
        for p in d["pending_positions"]:
            ...
            crossover_label = (
                ...
                if p["direction"] == "LONG"
                else f"Waiting for live price to fall to ${p['entry_price']:.2f}"
            )
            ...
            with st.container():
                ...
                with header_col:
                    ...
                    <div style="background:#13151f;border:1px solid rgba(255,167,38,0.25);border-radius:10px;padding:12px 16px;margin-bottom:4px;opacity:0.85">
                        ...
                        <span style="font-family:'JetBrains Mono',monospace;font-size:1rem;font-weight:700;color:#e2e8f0">{dir_icon} {p['ticker']}</span>
                        <span style="color:#6b7185;font-size:0.8rem;margin-left:10px">{p['company']}</span>&nbsp;&nbsp;
                        <span style="font-family:'JetBrains Mono',monospace;font-size:0.82rem;color:#9aa0b0">Entry target {fmt_usd(p['entry_price'])} · Current {fmt_usd(p['current_price'])} · {int(p['shares'])} shares · Cost {fmt_usd(p['cost_basis'])}</span><br>
                        <span style="font-size:0.75rem;color:#ffa726">⏳ {crossover_label}</span>
                        <span style="font-size:0.75rem;color:#6b7185"> · Exp {exp_text} · {days_held}d since entry</span>
                    </div>
                    """, unsafe_allow_html=True)
                    ...
                    if st.button("Activate", key=f"activate_btn_{p['trade_id']}", use_container_width=True):
                        ...
                        try:
                            ...
                            sh.append_row("Trade Log", [
                                ...
                                "Manually activated (overriding crossover check)"
                            ])
...
with right:
    ...
    fig_equity.add_trace(go.Scatter(
        x=dates, y=[sc] * len(dates), mode="lines", name="Starting Capital",
    ...
    fig_equity.update_layout(
        ...
        yaxis=dict(gridcolor="#1e2130", tickfont=dict(color="#6b7185", size=10), tickformat="$,.0f", showline=False),
        hovermode="x unified",
    )
    ...
    if d["closed_trades"]:
        ...
        fig_exit = go.Figure(go.Pie(
            labels=list(counts.keys()), values=list(counts.values()), hole=0.65,
            marker=dict(
                colors=["#00d4aa", "#ff6b6b", "#ffa726", "#5b9cf6", "#b18cfe", "#ff9cda"],
